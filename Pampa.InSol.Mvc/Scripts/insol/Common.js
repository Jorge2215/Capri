...
function recoverKendoDate(ctrl) {
    if (ctrl.data("kendoDatePicker").value() === '' || ctrl.data("kendoDatePicker").value() === 0 || ctrl.data("kendoDatePicker").value() === null) {
        return null;
    }
    return ctrl.data("kendoDatePicker").value().toISOString();
}
function recoverDdlObject(ctrl) {
    return (ctrl.data('kendoDropDownList').value() == '0') ? null : ctrl.data('kendoDropDownList').value()
}

function resizeGrid() {
    var gridWidth = $(".reports-grid table").width() - 5;

    $(".k-header.k-grid-toolbar.k-grid-top").width(gridWidth);
    $(".k-pager-wrap.k-grid-pager.k-widget.k-floatwrap").width(gridWidth);
}

function toggleInvalid(field, ctrl) {
    if (field == undefined || field == null || field == '' || field == 0) {
        ctrl.addClass(constants.invalidClass);
        return false;
    } else {
        ctrl.removeClass(constants.invalidClass);
        return true;
    }
}

function setDatetimePicker(ctrl) {
    ctrl.kendoDateTimePicker({
        format: "dd/MM/yyyy HH:mm",
        timeFormat: 'HH:mm',
    });
}

function setDropDownList(ctrl, data, textField, valueFiled) {
...
$.fn.dropDownList = function (opts) {
    if (opts) {
        this.kendoDropDownList({
            ...
            dataValueField: opts.valueFiled,
            dataSource: {
                data: opts.data,
            },
            enable: (opts.enabled != null && opts.enabled != undefined) ? opts.enabled : true,
            animation: false,
        });
        if (opts.change != undefined && opts.change != null) {
            this.data('kendoDropDownList').bind('change', opts.change);
        }
        if (opts.dataBound != undefined && opts.dataBound != null) {
            this.data('kendoDropDownList').bind('dataBound', opts.change);
        }
    } else {
        this.kendoDropDownList();
    }
...
$.fn.numericTextBox = function (opts) {
    if (opts) {
        this.kendoNumericTextBox({
            min: (opts.min ? opts.min : 0),
            max: (opts.max ? opts.max : 100),
            decimals: (opts.decimals ? opts.decimals : 0),
        });
    } else {
        this.kendoNumericTextBox();
    }
...
$.fn.tooltipPlugin = function (opts) {
    this.tooltipster({
        side: opts.side
    });
}
...
function GenerateBarcode(id, text) {
    return JsBarcode(id, text, { format: "CODE128", ean128: true });
}
...
function showSuccessNotification(title, message) {
    $('#myModal').modal('hide');
    $("#popupNotification").data("kendoNotification").hide();
    if (message != null) {
...
function createConfirm(msg, title, action, isInfoModal) {
    var kendoWindow = $('#confirmation').kendoWindow({
        resizable: false,
        modal: true
    });
    ...
    if (isInfoModal) {
        kendoWindow.find('.confirm-message').hide();
        kendoWindow.find('#msg.confirm-message').show();
        kendoWindow.find(".modal-confirm").text('Ok');
        kendoWindow.find(".modal-cancel").hide();
    }
}
...
function createModal(opts) {
    ...
    kendoWindow.data("kendoWindow").content($("#modal-content").html());
...
function setObservacionesModal(opts) {
    // {
    //	msg: '',
    // 	title: '',
    // 	isInfoModal: '',
    //	funcFillTxt: function(){},
    // }
    ...
    kendoWindow.data('kendoWindow').content($("#modal-confirmation").html());

    $('#msg').text(opts.msg);
    $('#confirmation_wnd_title').text(opts.title);
    ...
    if (opts.isInfoModal) {
        kendoWindow.find('.modal-body').append('<textarea id="txtObservaciones" class="k-textbox" cols="20" name="textarea" rows="5" style="width:100%;max-width:100%;" readonly="readonly"></textarea>');
        kendoWindow.find('.confirm-message').hide();
        kendoWindow.find(".modal-confirm").text('Ok');
        kendoWindow.find(".modal-cancel").hide();

        var txt = opts.funcFillTxt;
        kendoWindow.find('#txtObservaciones').text(txt);
    }
}
